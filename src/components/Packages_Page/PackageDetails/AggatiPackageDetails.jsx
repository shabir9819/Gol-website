...
export default function AggatiPackageDetails() {
  ...
  const Package__details_heading = () => (
    <>
      <ul className="package__details-heading flex flex-row flex-wrap w-full mb-[24px] rounded-[5px]">
        <li
          className={`flex-1 px-2 md:px-4 py-3 text-[12px] md:text-[16px] ${
            packageToggle === "overview" && "text-[#3A3A3A] font-bold"
          } leading-[22px] uppercase font-semibold relative  cursor-pointer text-center `}
          onClick={() => setPackageToggle("overview")}
        >
          overview{" "}
          {packageToggle === "overview" && (
            <span className="absolute bottom-0 left-0 w-full h-[5px] bg-[#3282AD] rounded-[8px]" />
          )}
        </li>
        <li
          className={`flex-1 px-2 md:px-4 py-3 text-[12px] md:text-[16px] ${
            packageToggle === "itinerary_details" && "text-[#3A3A3A] font-bold"
          } leading-[22px] uppercase font-semibold relative cursor-pointer text-center`}
          onClick={() => setPackageToggle("itinerary_details")}
        >
          itinerary
          {packageToggle === "itinerary_details" && (
            <span className="absolute bottom-0 left-0 w-full h-[5px] bg-[#3282AD] rounded-[8px] " />
          )}
        </li>
        ...
        <li
          className={`flex-1 px-2 md:px-4 py-3 text-[12px] md:text-[16px] ${
            packageToggle === "add_information" && "text-[#3A3A3A] font-bold"
          } leading-[22px] uppercase font-semibold relative cursor-pointer text-center`}
          onClick={() => setPackageToggle("add_information")}
        >
          additional info
          {packageToggle === "add_information" && (
            <span className="absolute bottom-0 left-0 w-full h-[5px] bg-[#3282AD] rounded-[8px] " />
          )}
        </li>
      </ul>
    ...
    <>
      ...
      <div className="package_details-desc pb-10 mb-12 border-b-2 border-b-[#A2A8AB]">
        ...
        <p className="text-[16px] text-[#5A5A5A] leading-[24px] font-normal mb-8">
          The Agatti Island is at a distance of 459 km (248 nautical miles) from
          Kochi and is located the west of Kavaratti Island. It lies between 10o
          48′ and 10o 53′ N latitude and 72o 09′ and 7o 13′ E longitude, having
          an area of 3.84 sq km, with a maximum length of 10 km and width of km.
          It has a north-east, south-west trend with a long tail on the south.
          The lagoon area of this island is 17.50 sq km. In Agatti coral growths
          and multicoloured coral fishes abound in its lagoons. Fishing is the
          most important industry of Agatti which is perhaps the only island
          besides Minicoy getting surplus fish. Next to Fishing, Coir and Copra
          are the main industries . Agatti is the Air Port stopover from Cochin
          on the way to Bangaram which can be seen on the northern horizon.
        </p>
        <p className="text-[16px] text-[#5A5A5A] leading-[24px] font-normal ">
          Agatti is one of the Lakshadweep islands open to tourism for domestic
          tourists. Visitors, however, are allowed to the Island under certain
          restrictions. They are required to obtain Entry Permit from the
          Lakshadweep Administration for entering or visiting the island. Entry
          Permit is issued based on the visitor having a confirmed place to stay
          with a sponsored Permit...
        </p>
      </div>
    </>
  );
  ...
  const Package__itenerary_details = () => (
    <>
      <div className="itinerary__details mb-[20px] md:mb-12 border-b-2 border-b-[#A2A8AB]">
        ...
        {itineraryDays_details.aggati.map((itinerary, _) => {
          return (
            <div
    ...
    <>
      <section className="package__details w-full flex flex-col lg:flex-row px-4 md:px-10  ">
        <div className="leftsection flex-1 flex flex-col pr-2 md:pr-[135px]">
          ...
          {packageToggle !== "add_information" && (
            <Package__itenerary_details />
          )}
          {packageToggle !== "itinerary_details" && (
            <Package__add_information />
          )}
        </div>
